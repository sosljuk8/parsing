...
func (pq *ProductQuery) QueryPage() *PageQuery {
	query := (&PageClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, false, product.PageTable, product.PageColumn),
		)
		fromU = sqlgraph.SetNeighbors(pq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (pq *ProductQuery) WithPage(opts ...func(*PageQuery)) *ProductQuery {
	query := (&PageClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if query := pq.withPage; query != nil {
		if err := pq.loadPage(ctx, query, nodes, nil,
			func(n *Product, e *Page) { n.Edges.Page = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProductQuery) loadCategories(ctx context.Context, query *CategoryQuery, nodes []*Product, init func(*Product), assign func(*Product, *Category)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(category.FieldID), joinT.C(product.CategoriesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(product.CategoriesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(product.CategoriesPrimaryKey[1]))
...
func (pq *ProductQuery) loadPage(ctx context.Context, query *PageQuery, nodes []*Product, init func(*Product), assign func(*Product, *Page)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Product)
	for i := range nodes {
		if nodes[i].product_page == nil {
			continue
		}
		fk := *nodes[i].product_page
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "product_page" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
