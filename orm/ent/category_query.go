...
func (cq *CategoryQuery) QueryChildren() *CategoryQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, category.ChildrenTable, category.ChildrenPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(cq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (cq *CategoryQuery) QueryParents() *CategoryQuery {
	query := (&CategoryClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(category.Table, category.FieldID, selector),
			sqlgraph.To(category.Table, category.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, false, category.ParentsTable, category.ParentsPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(cq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (cq *CategoryQuery) WithParents(opts ...func(*CategoryQuery)) *CategoryQuery {
	query := (&CategoryClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := cq.withParents; query != nil {
		if err := cq.loadParents(ctx, query, nodes,
			func(n *Category) { n.Edges.Parents = []*Category{} },
			func(n *Category, e *Category) { n.Edges.Parents = append(n.Edges.Parents, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CategoryQuery) loadProducts(ctx context.Context, query *ProductQuery, nodes []*Category, init func(*Category), assign func(*Category, *Product)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(category.ProductsTable)
		s.Join(joinT).On(s.C(product.FieldID), joinT.C(category.ProductsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(category.ProductsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(category.ProductsPrimaryKey[0]))
...
func (cq *CategoryQuery) loadChildren(ctx context.Context, query *CategoryQuery, nodes []*Category, init func(*Category), assign func(*Category, *Category)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(category.ChildrenTable)
		s.Join(joinT).On(s.C(category.FieldID), joinT.C(category.ChildrenPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(category.ChildrenPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(category.ChildrenPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (cq *CategoryQuery) loadParents(ctx context.Context, query *CategoryQuery, nodes []*Category, init func(*Category), assign func(*Category, *Category)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Category)
	nids := make(map[int]map[*Category]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(category.ParentsTable)
		s.Join(joinT).On(s.C(category.FieldID), joinT.C(category.ParentsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(category.ParentsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(category.ParentsPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
