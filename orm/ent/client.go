...
// QueryParents queries the parents edge of a Category.
func (c *CategoryClient) QueryParents(ca *Category) *CategoryQuery {
	query := (&CategoryClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := ca.ID
		step := sqlgraph.NewStep(
...
// QueryPage queries the page edge of a Product.
func (c *ProductClient) QueryPage(pr *Product) *PageQuery {
...
type (
	hooks struct {
		Category, Page, Product []ent.Hook
	}
	inters struct {
		Category, Page, Product []ent.Interceptor
	}
)
